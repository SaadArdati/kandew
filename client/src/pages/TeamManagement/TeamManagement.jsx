...
function Badge({ role }) {
  ...
  return <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${styles}`}>{role}</span>
}
...
export default function TeamManagement() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="bg-surface-container border border-outline rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4">
        ...
        {petalEditMode ? (
          <form onSubmit={handlePetalValueSave} className="flex gap-2 items-center">
            ...
            <button
              type="submit"
              className="bg-primary hover:bg-primary-container text-white px-3 py-1.5 rounded-xl text-xs font-semibold transition-colors"
            >
              Save
            </button>
            <button
              type="button"
              onClick={() => setPetalEditMode(false)}

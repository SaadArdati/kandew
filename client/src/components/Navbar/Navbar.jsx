...
export default function Navbar({ onLogout }) {
  ...
  const linkClass = ({ isActive }) =>
    `block px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-primary/15 text-primary' : 'text-on-surface-variant hover:text-on-surface'}`

  return (
    <nav className="bg-surface-container border-b border-outline/80 sticky top-0 z-50">
      ...
      {menuOpen && (
        <>
          <div
            ...
            aria-hidden="true"
          />
          <div className="md:hidden absolute left-0 right-0 top-14 z-50 bg-surface-container border-b border-outline/80 px-4 py-3 flex flex-col gap-0.5 shadow-lg">
            {/* Navigation */}
            <NavLink to="/app" end className={linkClass} onClick={() => setMenuOpen(false)}>
              Home
            </NavLink>
            <NavLink to="/app/tasks" className={linkClass} onClick={() => setMenuOpen(false)}>
              Tasks
            </NavLink>

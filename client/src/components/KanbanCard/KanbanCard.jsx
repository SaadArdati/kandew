...
import {buildPetalSlots, formatTaskDueDate, getTaskMaxPetals, getTaskPetals, isDueSoon,} from '../../utils/petalUtils';
...
function animatePetalCelebration(petalRow) {
    ...
    function addWobble(clone) {
        clone.animate(
            ...
            {
                duration: 200 + Math.random() * 100, iterations: Infinity,
                direction: 'alternate', composite: 'add', easing: 'ease-in-out'
            },
        );
    }
...
export default function KanbanCard({task, onDragStart, isDragging, isVictory, onClick, currentTime}) {
    ...
    return (
        ...
        >
            ...
            <div ref={petalRowRef} className="card-petal-row" aria-label={`${petals} of ${maxPetals} petals remaining`}>
                {petalSlots.map((slot) => (
                    <span
                        key={slot.key}
